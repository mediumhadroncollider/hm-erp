...
try {
    ...
    if (!is_string($inputPath) || trim($inputPath) === '') {
        throw new RuntimeException('Podaj ścieżkę wejściową: --input=/path/to/nasbi.csv');
    }
    ...
    $sourceDir = $monthDir . DIRECTORY_SEPARATOR . 'nasbi';
    ensureDir($sourceDir);

    fwrite(STDOUT, "Walidacja i parsowanie raportu nasbi za {$month}\n");
    fwrite(STDOUT, "Wejście: {$inputPath}\n\n");

    $result = validateAndParseNasbiCsv($inputPath, $inputOriginalName);
    if (($result['ok'] ?? false) !== true) {
        $message = (string)($result['message'] ?? 'Błędny raport nasbi.');
    ...
    writeJsonFile($outPath, [
        'snapshot_type' => 'nasbi_sales_by_isbn',
