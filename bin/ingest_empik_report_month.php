...
try {
    ...
    if (!is_string($inputPath) || trim($inputPath) === '') {
        throw new RuntimeException('Podaj ścieżkę wejściową: --input=/path/to/empik.csv');
    }
    ...
    $empikDir = $monthDir . DIRECTORY_SEPARATOR . 'empik';
    ensureDir($empikDir);

    fwrite(STDOUT, "Walidacja i parsowanie raportu Empik za {$month}\n");
    fwrite(STDOUT, "Wejście: {$inputPath}\n\n");

    $result = validateAndParseEmpikCsv($inputPath, $inputOriginalName);
    if (($result['ok'] ?? false) !== true) {
        $message = (string)($result['message'] ?? 'Błędny raport Empik.');
    ...
    $outPath = $empikDir . DIRECTORY_SEPARATOR . 'sales_by_isbn.json';
    writeJsonFile($outPath, [
        'snapshot_type' => 'empik_sales_by_isbn',
    ...
    $manifestPath = $empikDir . DIRECTORY_SEPARATOR . 'manifest.json';
    writeJsonFile($manifestPath, [
        'snapshot_type' => 'empik_manifest',
        'generated_at' => gmdate('c'),
        'month' => $month,
        'source' => 'empik',
        'input_file' => $inputPath,
